...
if (!userQuery && !hasPipedInput) {
  ...
  console.log(
    "   -s: 严格命令模式，仅返回命令，无任何解释，并提供粘贴执行选项",
  );
...
try {
  ...
  console.error(
    "获取系统信息失败，请确保已安装 fastfetch 并添加到 PATH 环境变量。",
  );
  systemInfo = "获取系统信息失败";
}
  ...
  try {
    ...
    if (fullText) {
      if (mode === "strict") {
        ...
        if (
          commandToExecute.startsWith("```") &&
          commandToExecute.endsWith("```")
        ) {
